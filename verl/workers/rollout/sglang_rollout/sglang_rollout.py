...
class ServerAdapter(BaseRollout):
    ...
    async def start_profile_auto_stop(self, **kwargs):
        ...
        """
        assert "num_steps" in kwargs.get("tags", {}), "Missing required 'num_steps' in tags for auto-stop profiling for sglang. "
    ...
    async def start_profile(self, tags: dict[str, any] = None, profile_ranks: list[int] = None):
        ...
        tags = tags or {}
        if (self.device_mesh["infer_tp"].get_local_rank() == 0 and
                self.device_mesh["dp"].get_local_rank() in profile_ranks):
