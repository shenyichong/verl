...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        # Collect model parameters for TFLOPs computation
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # compute throughput with TFLOPs if model params are available
                n_gpus = self.resource_pool_manager.get_n_gpus()
                metrics.update(
                    compute_throughout_metrics(
                        batch=batch, timing_raw=timing_raw, n_gpus=n_gpus, model_params_dict=self.model_params_dict
                    )
                )
